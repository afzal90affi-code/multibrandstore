...
import { collection, addDoc, serverTimestamp, onSnapshot } from "firebase/firestore"; // ✅ onSnapshot add kiya
...
export default function AddProduct() {
  ...
  const [cats, setCats] = useState<any[]>([]); // ✅ Categories state add ki
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (!form.title || !form.price || !form.image || !form.category) return alert("Title, Price, Category aur Image zaroor daalein");
    ...
    try {
      await addDoc(collection(db, "products"), {
        ...
        category: form.category.toLowerCase().trim(), // ✅ Selected category save hogi
      ...
      setForm({ title: "", price: "", category: "", collection: "", image: "", image2: "", sizes: [], inStock: true, featured: false });
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 text-gray-900 p-6">
      <div className="max-w-lg mx-auto flex-1">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-4">
          ...
          {/* ✅ CATEGORY DROPDOWN (Manual input ki jagah) */}
          <div>
          ...
          <input value={form.image2} onChange={(e) => setForm({...form, image2: e.target.value})} placeholder="Hover Image URL (For Beejays Effect)" className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-rose-500" />
          {form.image2 && (
            <div className="mt-4 rounded-3xl overflow-hidden border border-gray-200 bg-white shadow-sm">
          ...
          <button type="submit" disabled={loading} className="w-full bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 rounded-lg text-sm transition-colors disabled:opacity-50">
